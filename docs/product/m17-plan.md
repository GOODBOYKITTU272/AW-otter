...
## M17B addendum (implementation-complete, awaiting review)

M17B (Production Deployment + Scheduling Foundation) built and locally/Docker-verified everything expressible as code, without deploying anywhere real (no cloud account access):
- **Production Auth**: `/auth/set-password` (consumes Supabase's invite/recovery link via `detectSessionInUrl`, lets the user set a password, then redirects home) closes the M17A-flagged gap. Found and fixed a SECOND, more complete gap while implementing it — the in-app `/admin/people/new` flow (`createPerson`) never actually sent an invite at all (only inserted a `status: 'invited'` row, no `auth.users`, no email) — every AM/Manager added this way beyond the very first admin was previously unreachable. New `inviteMembership` (service-role, narrowest-possible-privilege) closes this. `bootstrap-admin.mjs` now passes `redirectTo` — without it, the on-auth-user-created trigger activates a membership at invite time, and the invited admin would land on an "already active" page having never set a password, with no way back in.
- **A stale-but-harmless self-correction**: initially believed (per the M17A doc) that no session-refresh middleware existed. It does — `proxy.ts`, under Next.js 16's renamed convention — the M17A finding was reading an outdated comment (`server.ts` said "middleware.ts"), not a real gap. Caught by the build itself failing on a duplicate file; both docs and the stale comment are now corrected.
- **Microsoft subscription renewal**: `renewExpiringMicrosoftSubscriptions` (new, `packages/domain/src/microsoft-subscription-renewal.ts`) wires the M17A-identified gap — CAS-claimed (no duplicate renewals), org-scoped, a 404 (subscription already gone) escalates to a critical incident instead of retrying forever. 6 new tests including a concurrent-claim race test.
- **Scheduling**: every `/api/internal/*` route now accepts a scheduler's native `Authorization: Bearer <CRON_SECRET>` alongside the original `x-internal-queue-secret`, via a new shared `isAuthorizedInternalRequest` helper — zero duplicated auth logic across 11 routes. `vercel.json` has the concrete cron entries. `/api/internal/transcription/process` deliberately has no GET/cron wiring — see below.
- **Transcription worker**: `workers/transcription-worker/` (script + Dockerfile), reusing the exact same domain functions the route already calls. Built and ran the actual Docker image during this checkpoint — confirmed ffmpeg/ffprobe present and runnable, confirmed the worker script resolves all modules and starts its tick loop correctly inside the container. **Found and fixed a real, previously-undiscovered bug in already-shipped M6 code while doing this**: `workers/orchestrator/bot-worker.mjs` has never actually run successfully via the `node` command its own header comment has documented since M6 — plain Node's ESM loader can't resolve the extensionless relative imports inside `@applywizz/domain`'s own source files. Confirmed by literally running it. Both workers now correctly use `tsx` (already a repo devDependency); re-verified both get past module resolution afterward.
- **Production health**: new public `/api/health` (DB-reachability check, matches conventional uptime-monitor wiring — no secret required, reveals nothing beyond ok/degraded). Everything else §8 asked about (queue backlog, stuck jobs, open incidents, subscription-renewal failures) was already covered by M16 + this checkpoint's own incident wiring — deliberately not rebuilt.
- **Full detail**: `docs/ops/production-scheduling.md` (the per-job cadence/batch/timeout/retry/alert/owner table §6 required).
- **Zero migrations** — confirmed via `supabase db diff` (clean) and `git diff --stat main -- supabase/` (empty). Everything in M17B is application code + deployment config.
- **Still open, unchanged from M17A**: real Supabase/Vercel/Entra account provisioning (no cloud credentials available to this session), hosting platform final confirmation, SMTP configuration, all M17A business decisions (retention, consent, email provider, default-join policy).
